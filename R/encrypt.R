...
#' @param keyid The key that has to be used when signing and encrypting the file. Default is \code{NULL} and hence, the default values will be taken.
...
encrypt <- function(input, output = NULL, passphrase = NULL, compress = "ZLIB",
  ...
  # Key ID
  if (!is.null(keyid)) {
    keyid = paste0("-se --recipient ", keyid," --local-user ",keyid)
  } else {
    keyid = NULL
  }
  ...
  if (is.null(passphrase)) {
    ...
    system2.args <- c(
      ...
      "--passphrase-fd 0",
      "--batch",
      tty, keyid,"--output", output, "--symmetric", armor, mdc,
      paste("--compress-algo", compress),
      paste("--cipher-algo", cipher),
      paste("--s2k-mode", s2k.mode),
      paste("--s2k-digest-algo", s2k.digest),
